...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const authToken = generateToken(employee.Employee_Id);
        res.setHeader('Set-Cookie', `authToken=${authToken}; HttpOnly; Path=/; Max-Age=3600`);
        return res.status(200).json({ message: 'Амжилттай нэвтэрлээ', employee });
    } catch (error) {
        console.error('Error logging in:', error);
        return res.status(500).json({
